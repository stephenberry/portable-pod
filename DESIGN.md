...
**Every field is bounded and proved on its own**, never merged with another field whose type is spelled the same: identical tokens can name different types (§12 has the case, and the release it broke).
...
The same investigation found a second hygiene bug in the same place. The derive moved the `#[pod(crate = ...)]` path onto each field's span to point diagnostics at the field, and `$crate` resolves through its span, so on a field written by another crate's macro the path named that crate. A user-given path now keeps its own spans. Only the default `::portable_pod`, the derive's own token, is moved, which keeps every field diagnostic where it was (§5.2); with `crate = ...`, rustc draws the span from the path to the field instead (`tests/ui/pod_crate_field_not_pod.rs`).
...
One sound saving remains and is not taken. A field type whose tokens mention none of the struct's generic parameters and no `Self` needs no where-clause bound, because the inherited proof's `<F as Pod>::__LAYOUT_OK` already requires `F: Pod` and fails at the definition when it is not. Dropping those bounds would save about a third of the per-field cost. It would also move the non-`Pod` diagnostics from the impl's where clause into the proof and change every `field_*` golden, and for a generic struct it would turn §3's use-site error into a definition-site one. That is a separate decision.
