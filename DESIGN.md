...
Where the proof sits matters for that count, because rustc reports a failed obligation once per *body*, not once per program. A concrete type's `SHAPE` has to name each field's shape, so it names `<Field as Pod>` anyway; the fields' inherited layout proofs are put in the same body, beside those projections and on the same span, and are reported together, once. `__LAYOUT_OK` then forces `SHAPE`, so it is still the whole transitive proof. Left in `__LAYOUT_OK`, the inherited proofs reported every non-`Pod` field twice, identically. The padding goldens did not move: the forcing is spanned at the derive attribute, where the padding trail already points.

This changes no accepted program. For a type with no generic parameters, a bound `F: Pod` holds exactly when the body's `<F as Pod>` is well-formed, so the same set of types derives, with the same impl, the same `__LAYOUT_OK` and the same `SHAPE` (`golden_derive_forms` in `tests/shape.rs` and `golden_shapes` in `tests/cross_crate.rs` pinned representative forms before the change). Only where and how often a rejected program is reported moved: the `field_*` goldens lost the error each used to carry at the struct's name, a field type of several tokens is now underlined whole, and `cross_crate_non_pod_field.stderr` lost the errors at the macro call and at its `read_pod`.
...
**The emitted const**, and what it cost. It is one expression, lazily evaluated (rustc evaluates an associated const only where something names it), with no panic site: a const panic site costs at every derive (§5.1). A generic type's has no layout proof in it either; forcing `__LAYOUT_OK` would be redundant beside the entry points that already force it, and would make reading an instantiation's shape a layout check. A concrete type's `SHAPE` carries the fields' inherited proofs and is forced by `__LAYOUT_OK`, so that each field is named `Pod` in one body and a field that is not `Pod` is one error (§3); it is evaluated at every concrete definition, which the measurement below found free. Every path in it is absolute (`::core::primitive::u64`, `::core::option::Option`), so a `type u64 = u32;` in the user's scope changes nothing; `shadowed_names` in `tests/shape.rs` pins that, and the size pins' `usize` got the same treatment.
...
**What it refuses rather than drops.** The attribute says the newtype and its field are one thing to anything reading a shape, so anything the default fold would add on top is an error: a second field, even a zero-sized one whose name the fold would record (`tests/ui/transparent_two_fields.rs`); a `shape_with` (`transparent_shape_with.rs`); and a const parameter that the field's type does not mention (`transparent_unused_const_param.rs`), because the default derive folds `Fixed<32>` and `Fixed<24>` apart and forwarding would merge them. A parameter the field does mention reaches the shape through it, as a type parameter always does: `Lanes<3>([u16; 3])` has `[u16; 3]`'s shape. That test is token-level, like everything in the parser (§5): a const parameter counts as mentioned when its identifier appears anywhere in the field's type.
